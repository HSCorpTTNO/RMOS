...
static acpi_rsdp_desc_t* rsdt = NULL;
static acpi_xsdp_desc_t* xsdt = NULL;
...
void init_setup_acpi(struct stivale2_struct* stivale2_struct, uint64_t id) {
    ...
    acpi_xsdp_desc_t* root = (acpi_xsdp_desc_t*)rsdp_tag->rsdp;

    // We can use XSDT since this hardware is not legacy!
    if (root->rev > 0) {
        using_xsdt = 1;
        xsdp = (acpi_xsdt_desc_t*)root->xsdt;
    } else {
        // This hardware is legacy and requires rsdp.
        rsdt = (acpi_rsdp_desc_t*)root->rsdt;
    }
}
